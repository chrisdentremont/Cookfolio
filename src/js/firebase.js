...
onAuthStateChanged(auth, (user) => {
  ...
  if(window.location.href.includes("recipes")){ //Display recipes if user is on the recipes page
    getDoc(doc(db, "users", user.uid)).then((res) => {
      displayRecipes(res.data());
    })
  }else if(window.location.href.includes("ingredients")){ //Display ingredients if user is on the ingredients page
    getDoc(doc(db, "users", user.uid)).then((res) => {
...
function signIn(email, password){
  signInWithEmailAndPassword(auth, email, password).then(() => {
    ...
    if(loginPassWarning.firstChild) {loginPassWarning.removeChild(loginPassWarning.firstChild);} //Prevent duplicate messages
